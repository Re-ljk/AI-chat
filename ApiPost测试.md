...
```json
{
	...
	"data": {
		...
		"user_id": "1fe8b02b-fb2c-11f0-8e56-0c9a3c9f30ca",
		"session_name": "新会话",
		"is_active": true,
		"created_at": "2026-01-27T03:30:00+00:00",
		"updated_at": null
	}
}
```
...
3. 设置请求URL: `http://localhost:19088/api/v1/sessions/{session_id}`
   - 将 `{session_id}` 替换为测试6（创建会话接口）返回的会话ID
4. 设置请求头:
   - `Authorization: Bearer {access_token}`
   - 将 `{access_token}` 替换为测试2（登录接口）获取的token
5. 点击发送请求
...
```json
{
	...
	"data": {
		...
		"is_active": false,
		"created_at": "2026-01-27T03:30:00+00:00",
		"updated_at": "2026-01-27T03:35:00+00:00"
	}
}
```
...
```json
{
	...
	"data": [
		...
		},
		{
			...
			"session_name": "会话2",
			"is_active": true,
			"created_at": "2026-01-27T03:40:00+00:00",
			"updated_at": null
		}
	]
}
